...
app.config[
    'SQLALCHEMY_DATABASE_URI'] = 'mysql://root:R33dxq2!!zghj@localhost/neu_studienverlaufsplan'  # hier Passwort der DB und den Namen der DB eingeben
db = SQLAlchemy(app)
...
class RegisterForm(FlaskForm):
    ...
    erste_Vertiefung = SelectField('Vertiefung 1',
                                   choices=choices)  # ('Embedded Systems', 'Visual Computing', 'Complex and Intelligent Software Systems', 'Medizinische Informatik'))
    zweite_Vertiefung = SelectField('Vertiefung 2',
                                    choices=choices)  # ('Embedded Systems', 'Visual Computing', 'Complex and Intelligent Software Systems', 'Medizinische Informatik'))
...
def signup():
    ...
    if form.validate_on_submit():
        ...
        new_user = benutzer(matrikelnummer=form.username.data, vorname=form.vorname.data, nachname=form.nachname.data,
                            email=form.email.data, passwort=hashed_password,
                            wahlvertiefung_ID=form.erste_Vertiefung.data,
                            wahlvertiefung2_ID=form.zweite_Vertiefung.data,
                            immatrikulationssemester=form.immatrikulationssemester.data,
                            immatrikulationsjahr=form.immatrikulationsjahr.data)
...
def modulauswahl():
    ...
    semesterwochenstunden = int(
        dbase.get_Summe_Pflicht_Vertiefung_sws(user_wahlvertiefung_ID, user_id, current_semester)[0][0]) + \
    ...
    if user_pflicht_lp_ist != user_pflicht_lp_soll:
        ...
        #Bachelorarbeit erst ab 120LP erscheinen lassen
        if lp_gesamt < 120:
            for i in module1:
                if i[2] == "Bachelorarbeit":
                    module1.remove(i)
        pflichtkurse_nicht_empfohlen = dbase.get_module_nicht_empfohlen_pflicht(start_semester, current_semester,
                                                                                "Pflicht", user_id)
        pflichtkurse_vertiefung_empfohlen = dbase.get_VertiefungPflichtModule(start_semester, current_semester,
                                                                              user_wahlvertiefung_ID, user_id)
        pflichtkurse_vertiefung_nicht_empfohlen = dbase.get_nichtEmpfohleneVertiefungPflichtModule(start_semester,
                                                                                                   current_semester,
                                                                                                   user_wahlvertiefung_ID,
                                                                                                   user_id)
    else:
        module1 = []
    ...
    if user_wahlvertiefung_ID == 1 or user_wahlvertiefung_ID == 3:
        if user_grundlagenpraktikum_lp_ist == 0:
            grundlagenpraktikum = dbase.get_Grundlagenpraktikum_zu_Vertiefung(start_semester, current_semester,
                                                                              user_wahlvertiefung_ID, user_id)
            grundlagenpraktikum_nicht_empfohlen = dbase.get_Grundlagenpraktikum_zu_Vertiefung_nicht_empfohlen(
                start_semester, current_semester, user_wahlvertiefung_ID, user_id)
        else:
            grundlagenpraktikum = []
    ...
    if user_weitere_einfuehrung_LP_ist != user_weitere_einfuehrung_LP_soll:
        zweites_Grundlagenmodul = dbase.get_Einfuehrung_zu_Vertiefung2(start_semester, current_semester,
                                                                       user_wahlvertiefung2_ID, user_id)
        zweites_Grundlagenmodul_nicht_empfohlen = dbase.get_Einfuehrung_zu_Vertiefung3(start_semester, current_semester,
                                                                                       user_wahlvertiefung2_ID, user_id)
    # Wahlpflichtbereich
    if user_wahlvertiefung_ID == 1 or user_wahlvertiefung_ID == 3:
        if user_wahlpflicht_LP_ist + user_wahlpflicht_andere_LP_ist < 30:
            empfohlene_wahlpflichtkurse = dbase.get_VertiefungModule(start_semester, current_semester,
                                                                     user_wahlvertiefung_ID, user_id)
            nichtempfohlene_wahlpflichtkurse = dbase.get_nichtEmpfohleneVertiefungModule(start_semester,
                                                                                         current_semester,
                                                                                         user_wahlvertiefung_ID,
                                                                                         user_id)
            if user_wahlpflicht_andere_LP_ist < 12:
                andere_empfohlene_wahlpflichtkurse = dbase.get_andereModule(start_semester, current_semester,
                                                                            user_wahlvertiefung_ID, user_id)
                andere_nichtempfohlene_wahlpflichtkurse = dbase.get_nichtEmpfohleneAndereModule(start_semester,
                                                                                                current_semester,
                                                                                                user_wahlvertiefung_ID,
                                                                                                user_id)
            else:
        ...
        if user_wahlvertiefung_ID == 1 or user_wahlvertiefung_ID == 3:
            for i in empfohlene_wahlpflichtkurse:
                if (int(idModule) == i[0]):
                    ...
                    if (user_wahlpflicht_LP_ist + user_wahlpflicht_andere_LP_ist + temp_wahlpflicht_lp > 30) or (
                            user_wahlpflicht_LP_ist + temp_wahlpflicht_lp > 30):
            ...
            for i in andere_empfohlene_wahlpflichtkurse:
                if (int(idModule) == i[0]):
                    temp_wahlpflicht_lp = int(elementFromDB[0][6])
                    if (user_wahlpflicht_LP_ist + user_wahlpflicht_andere_LP_ist + temp_wahlpflicht_lp > 30) or (
                            user_wahlpflicht_andere_LP_ist + temp_wahlpflicht_lp > 12):
                        flash("Wahl des Modules mit ID " + idModule + " nicht möglich.")
                        # TODO Flash Wahl des Moduls mit ID: idModule nicht möglich
                        pass
            ...
            for i in empfohlene_wahlpflichtkurse:
                if (int(idModule) == i[0]):
                    temp_wahlpflicht_lp = int(elementFromDB[0][6])
                    if (user_wahlpflicht_LP_ist + user_wahlpflicht_andere_LP_ist + temp_wahlpflicht_lp > 12) or (
                            user_wahlpflicht_LP_ist + temp_wahlpflicht_lp > 12):
                        flash("Wahl des Modules mit ID " + idModule + " nicht möglich.")
                        # TODO Flash Wahl des Moduls mit ID: idModule nicht möglich
                        pass
            for i in nichtempfohlene_wahlpflichtkurse:
            ...
            for i in andere_empfohlene_wahlpflichtkurse:
                if (int(idModule) == i[0]):
                    temp_wahlpflicht_lp = int(elementFromDB[0][6])
                    if (user_wahlpflicht_LP_ist + user_wahlpflicht_andere_LP_ist + temp_wahlpflicht_lp > 12) or (
                            user_wahlpflicht_andere_LP_ist + temp_wahlpflicht_lp > 6):
                        flash("Wahl des Modules mit ID " + idModule + " nicht möglich.")
                        # TODO Flash Wahl des Moduls mit ID: idModule nicht möglich
                        pass
            for i in andere_nichtempfohlene_wahlpflichtkurse:
